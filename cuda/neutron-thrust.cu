...
#define OUTPUT_FILE "/tmp/3504927/absorbed.dat"
...
__global__ void neutron_gpu(int n,int* r,int* t,int* b, float* absorbed,float c, float c_c, float c_s, float h){
  ...
  float L, d, u, x;
  // L : distance parcourue par le neutron avant la collision
  // d : direction du neutron (0 <= d <= PI)
  // u : variable aléatoire uniforme
  // x : position de la particule (0 <= x <= h)
...
int main(int argc, char *argv[]) {
  ...
  NB_BLOCK = (n + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
