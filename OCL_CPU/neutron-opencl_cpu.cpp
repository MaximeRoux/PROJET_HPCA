...
int main(int argc, char *argv[]) {
	...
	start = my_gettimeofday();
  	try {
		...
		cl::Kernel add(program, "neutron_cpu");

	
		//start = my_gettimeofday();
		// Allocation et transfert sur cpu 

		cl::Buffer cpu_r(context, CL_MEM_COPY_HOST_PTR,sizeof(int),&r);


		cl::Buffer cpu_t(context, CL_MEM_COPY_HOST_PTR,sizeof(int),&t);
	
	

		cl::Buffer cpu_b(context, CL_MEM_COPY_HOST_PTR,sizeof(int),&b);



		cl::Buffer g_absorbed(context, CL_MEM_COPY_HOST_PTR,absorbed.size() * sizeof(float),absorbed.data());
		...
		queue.enqueueReadBuffer(cpu_r, CL_TRUE, 0, sizeof(int), &r);
		queue.enqueueReadBuffer(cpu_b, CL_TRUE, 0, sizeof(int), &b);
		queue.enqueueReadBuffer(cpu_t, CL_TRUE, 0, sizeof(int), &t);
